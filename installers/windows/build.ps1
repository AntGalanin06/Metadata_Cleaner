...
function Test-BuildFiles {
    ...
    $requiredFiles = @(
        (Join-Path $DIST_DIR "MetadataCleaner.exe"),
...
function Build-Installer {
    ...
    # Execute NSIS from the script directory
    Write-Info "Running NSIS with arguments: $($nsisArgs -join ' ')"
    Write-Info "Working directory: $SCRIPT_DIR"
    
    try {
        $process = Start-Process -FilePath "makensis" -ArgumentList $nsisArgs -NoNewWindow -Wait -PassThru -WorkingDirectory $SCRIPT_DIR
        
        if ($process.ExitCode -eq 0) {
            Write-Info "NSIS completed successfully with exit code: $($process.ExitCode)"
            Write-Info "Checking for installer at: $outputPath"
            ...
            if (Test-Path $outputPath) {
                ...
                Write-Error "NSIS completed but installer file not found at: $outputPath"
                Write-Info "Checking dist directory contents:"
                if (Test-Path $DIST_DIR) {
                    Get-ChildItem $DIST_DIR | ForEach-Object { Write-Info "  - $($_.Name)" }
                } else {
                    Write-Error "Dist directory does not exist: $DIST_DIR"
                }
                return $false
            }
        } else {
