...
def run_command(
    cmd: List[str], description: str = "", check_result: bool = True
) -> Tuple[bool, str]:
...
def check_build_system(project_root: Path) -> bool:
    ...
    build_files = ["build.py", "pyproject.toml", "Makefile"]
...
def test_basic_functionality(project_root: Path) -> bool:
    ...
    for import_test in test_imports:
        success, _ = run_command(
            [sys.executable, "-c", import_test], f"Импорт: {import_test}", False
        )
    ...
    success, _ = run_command(
        [sys.executable, "-m", "pytest", "tests/test_models.py", "-v", "--tb=short"],
        "Базовые тесты",
        False,
    )
...
def generate_summary_report(project_root: Path, checks: dict) -> None:
    ...
    if all(checks.values()):
        ...
        print(
            f"{Color.GREEN}✨ Проект Metadata Cleaner готов к использованию!{Color.END}"
        )
        ...
        print(
            f"{Color.YELLOW}Исправьте указанные выше ошибки перед использованием{Color.END}"
        )
