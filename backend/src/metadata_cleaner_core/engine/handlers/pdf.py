...
from metadata_cleaner_core.engine.errors import BackupError, EncryptedFileError
...
class PDFHandler(BaseHandler):
    ...
    def _clean_pdf_metadata(self, job: FileJob) -> dict[str, Any]:
        ...
        # Получаем clean_fields с безопасной проверкой на None
        clean_fields = job.clean_fields or {}
        ...
        if reader.metadata:
            ...
            if clean_fields.get("title", False) and metadata.title:
                cleaned_fields["title"] = metadata.title

            if clean_fields.get("author", True) and metadata.author:
                cleaned_fields["author"] = metadata.author

            if clean_fields.get("subject", False) and metadata.subject:
                cleaned_fields["subject"] = metadata.subject

            if clean_fields.get("creator", True) and metadata.creator:
                cleaned_fields["creator"] = metadata.creator

            if (
                clean_fields.get("keywords", False)
                and hasattr(metadata, "keywords")
                and metadata.keywords
            ):
                cleaned_fields["keywords"] = metadata.keywords

            if clean_fields.get("created", True) and metadata.creation_date:
                cleaned_fields["created"] = str(metadata.creation_date)

            if clean_fields.get("modified", True) and metadata.modification_date:
                cleaned_fields["modified"] = str(metadata.modification_date)

            # Producer - производитель PDF
            if (
                clean_fields.get("producer", True)
        ...
        if not self._should_remove_all_metadata(clean_fields):
            # Селективная очистка - создание новых метаданных
            new_metadata: dict[str, Any] = {}

            if reader.metadata:
                if not clean_fields.get("title", False) and reader.metadata.title:
                    new_metadata["/Title"] = reader.metadata.title

                if not clean_fields.get("author", True) and reader.metadata.author:
                    new_metadata["/Author"] = reader.metadata.author

                if not clean_fields.get("subject", False) and reader.metadata.subject:
                    new_metadata["/Subject"] = reader.metadata.subject

                if not clean_fields.get("creator", True) and reader.metadata.creator:
                    new_metadata["/Creator"] = reader.metadata.creator

                if (
                    not clean_fields.get("keywords", False)
                ...
                # Producer - производитель PDF
                if (
                    not clean_fields.get("producer", True)
