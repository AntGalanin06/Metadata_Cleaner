    ...
    def _clean_jpeg_exif(self, job: FileJob) -> dict[str, Any]:
        ...
        try:
            ...
            any_cleaning_enabled = any(
                value for value in clean_fields.values() if isinstance(value, bool)
            )
            ...
            camera_fields_enabled = any(
                [
                    clean_fields.get("exif_camera", False),
                    clean_fields.get("exif_author", False),
                    clean_fields.get("exif_software", False),
                    clean_fields.get("camera_owner", False),
                    clean_fields.get("camera_serial", False),
                    clean_fields.get("camera", False),  # для обратной совместимости
                ]
            )
            ...
            gps_fields_enabled = any(
                [
                    clean_fields.get("gps_coords", False),
                    clean_fields.get("gps_altitude", False),
                    clean_fields.get("gps", False),  # для обратной совместимости
                ]
            )
            ...
            date_fields_enabled = any(
                [
                    clean_fields.get("exif_datetime", False),
                    clean_fields.get("created", False),  # для обратной совместимости
                ]
            )
