...
from metadata_cleaner_core.engine.errors import BackupError, MetadataProcessingError
...
class OfficeHandler(BaseHandler):
    ...
    def _clean_docx(self, job: FileJob) -> dict[str, Any]:
        ...
        if clean_fields.get("author", True) and props.author:
            cleaned_fields["author"] = props.author
            props.author = ""

        if clean_fields.get("creator", True):
            if hasattr(props, "creator") and props.creator:
                cleaned_fields["creator"] = props.creator
                props.creator = ""

        if clean_fields.get("title", False) and props.title:
            cleaned_fields["title"] = props.title
            props.title = ""

        if clean_fields.get("subject", False) and props.subject:
            cleaned_fields["subject"] = props.subject
            props.subject = ""

        if clean_fields.get("keywords", False) and props.keywords:
            cleaned_fields["keywords"] = props.keywords
            props.keywords = ""

        if clean_fields.get("comments", True) and props.comments:
            cleaned_fields["comments"] = props.comments
            props.comments = ""

        if clean_fields.get("created", True) and props.created:
            cleaned_fields["created"] = str(props.created)
            # Для дат просто сохраняем информацию, но не изменяем

        if clean_fields.get("modified", True) and props.modified:
            cleaned_fields["modified"] = str(props.modified)
            # Для дат просто сохраняем информацию, но не изменяем

        if clean_fields.get("revision", True) and props.revision:
    ...
    def _clean_pptx(self, job: FileJob) -> dict[str, Any]:
        ...
        if clean_fields.get("revision", True):
    ...
    def _clean_xlsx(self, job: FileJob) -> dict[str, Any]:
        ...
        if clean_fields.get("creator", True) and props.creator:
            cleaned_fields["creator"] = props.creator
            props.creator = ""
        ...
        if clean_fields.get("comments", True) and props.description:
