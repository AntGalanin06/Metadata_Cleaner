...
class MetadataRegistry:
    ...
    @staticmethod
    def _field_to_payload(field: MetadataField) -> dict[str, Any]:
        """Преобразовать MetadataField в сериализуемый словарь."""
        return {
            "key": field.key,
            "category": field.category.value,
            "name_key": field.name_key,
            "description_key": field.description_key,
            "result_fields": list(field.result_fields),
            "default_remove": field.default_remove,
            "priority": field.priority,
        }

    @classmethod
    def get_catalogue(cls) -> list[dict[str, Any]]:
        """Получить каталог полей метаданных по типам файлов."""
        catalogue: list[dict[str, Any]] = []
        for file_type in cls.get_supported_file_types():
            fields = [
                cls._field_to_payload(field) for field in cls.get_fields_for_file_type(file_type)
            ]
            catalogue.append({"file_type": file_type, "fields": fields})
        return catalogue

    @classmethod
    def get_categories(cls) -> list[str]:
        """Получить список доступных категорий метаданных."""
        return [category.value for category in MetadataCategory]
