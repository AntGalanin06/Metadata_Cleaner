...
class PlatformBuilder:
    ...
    def run_command(
        ...
        try:
            ...
            logger.exception(f"Exit code: {e.returncode}")
            if e.stdout:
...
"""
    ...
    def _verify_build(self) -> None:
        ...
        if self.system == "Darwin":
            ...
            # Windows creates single EXE file
            exe_path = DIST_DIR / f"{APP_NAME}.exe"
            if not exe_path.exists():
                raise BuildError(f"Windows executable not created: {exe_path}")
            logger.info(f"Windows executable built successfully: {exe_path}")
        else:
            # Linux creates directory with executable
    ...
    def run_tests(self) -> None:
        ...
        if self.system == "Darwin":
            ...
            # Windows creates single EXE file
            exe_path = DIST_DIR / f"{APP_NAME}.exe"
            app_path = exe_path  # For Windows, the exe is the "app"
        else:
            # Linux creates directory with executable
            app_path = DIST_DIR / APP_NAME
            exe_path = app_path / APP_NAME
        ...
        if exe_path.exists():
            ...
            MIN_EXPECTED_SIZE_MB = 10
            MAX_EXPECTED_SIZE_MB = 500

            if size_mb < MIN_EXPECTED_SIZE_MB:
                logger.warning("Executable seems unusually small")
            elif size_mb > MAX_EXPECTED_SIZE_MB:
