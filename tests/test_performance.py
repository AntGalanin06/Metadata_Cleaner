...
class TestPerformance:
    ...
    def test_batch_processing_time(self, performance_dispatcher, large_temp_dir):
        ...
        print(
            f"Batch processing: {total_time:.3f}s total, {avg_time_per_file:.3f}s per file"
        )
    ...
    def test_large_file_processing(self, performance_dispatcher, temp_dir):
        ...
        print(
            f"Large file ({len(large_data)/1024/1024:.1f}MB) processing time: {processing_time:.3f}s"
        )
    ...
    def test_scalability(self, performance_dispatcher, temp_dir, file_count):
        ...
        print(
            f"Files: {file_count}, Total time: {total_time:.3f}s, Per file: {time_per_file:.3f}s"
        )
...
class TestPerformanceBenchmarks:
    ...
    def test_baseline_performance(self, performance_dispatcher, temp_dir):
        ...
        assert all(
            data["throughput"] > 10 for data in results.values()
        )  # Больше 10 KB/s
