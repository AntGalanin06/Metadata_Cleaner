...
class TestEndToEndWorkflow:
    ...
    def test_complete_image_processing_workflow(
        self, dispatcher, sample_files, temp_dir
    ):
    ...
    def test_different_output_modes(self, dispatcher, sample_files, temp_dir):
        ...
        for mode, mode_name in test_modes:
            ...
            with patch.object(
                dispatcher.settings_service, "get_output_mode", return_value=mode
            ):
                # Обрабатываем файл
    ...
    def test_concurrent_file_access(self, dispatcher, temp_dir):
        ...
        def process_file_thread():
            ...
            try:
                ...
                thread_file = (
                    temp_dir / f"thread_{threading.current_thread().ident}.jpg"
                )
        ...
        assert all(
            r.status in [CleanStatus.SUCCESS, CleanStatus.SKIPPED, CleanStatus.ERROR]
            for r in results
        )
...
class TestGUIIntegration:
    ...
    def test_gui_file_processing_integration(
        self, mock_flet_page, sample_files, temp_dir
    ):
