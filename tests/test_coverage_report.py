...
class TestCoverageReporting:
    ...
    def test_coverage_thresholds(self):
        ...
        minimum_coverage = 40  # 40% минимальное покрытие
        ...
        assert minimum_coverage >= 30  # Разумный минимум
    ...
    def test_coverage_integration_with_ci(self):
        ...
        if workflows_dir.exists():
            ...
            coverage_indicators = ["coverage", "codecov", "--cov", "pytest-cov"]

            has_coverage = any(
                indicator in ci_content.lower() for indicator in coverage_indicators
            )
    ...
    def test_coverage_badges_configuration(self):
        ...
        readme_files = [project_root / "README.md", project_root / "README_EN.md"]

        coverage_badge_indicators = ["codecov", "coverage", "badge", "shield"]
        ...
        for readme_file in readme_files:
            if readme_file.exists():
                ...
                has_badges = any(
                    indicator in content for indicator in coverage_badge_indicators
                )
    ...
    def test_coverage_config_completeness(self):
        ...
        if pyproject_file.exists():
            ...
            for option in expected_options:
                assert (
                    option in content
                ), f"Опция покрытия {option} не найдена в конфигурации"
    ...
    def test_coverage_reporting_performance(self):
        ...
        if result.returncode == 0:
            ...
            assert (
                execution_time < 30
            ), f"Генерация отчета покрытия слишком медленная: {execution_time:.1f}s"
    ...
    def test_coverage_exclusion_markers(self):
        ...
        exclusion_markers = ["# pragma: no cover", "# nocov", "# coverage: disable"]
...
class TestCoverageQuality:
        ...
        for module_path in critical_modules:
            ...
            if full_path.exists():
                ...
                if full_path.is_file():
                    ...
                    assert (
                        test_file.exists()
                    ), f"Тест для критического модуля {module_name} не найден"
    ...
    def test_test_files_naming_convention(self):
        ...
        if tests_dir.exists():
            ...
            if non_test_files:
                pytest.fail(
                    f"Файлы в tests/ не следуют соглашению именования: {[f.name for f in non_test_files]}"
                )
    ...
    def test_test_functions_coverage_quality(self):
        ...
        if tests_dir.exists():
            ...
            ), f"Недостаточно тестовых функций: {total_test_functions}"
