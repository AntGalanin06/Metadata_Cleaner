    ...
    def test_main_with_nonexistent_file(self):
        ...
        with patch('sys.argv', ['metadata_cleaner', '/nonexistent/file.jpg']):
            with patch('metadata_cleaner.cli.MetadataDispatcher') as mock_dispatcher:
                mock_dispatcher.return_value.clean_file.return_value = {'status': 'error', 'message': 'File not found'}
                result = main()
    ...
    def test_main_with_valid_arguments(self):
        ...
        try:
            ...
            with patch('sys.argv', ['metadata_cleaner', str(temp_path)]):
                with patch('metadata_cleaner.cli.MetadataDispatcher') as mock_dispatcher_class:
                    ...
                    from metadata_cleaner.cleaner.models import CleanStatus, CleanResult, FileJob
                    
                    # Создаем успешный результат
                    job = FileJob(file_path=Path(str(temp_path)))
                    mock_result = CleanResult(job=job, status=CleanStatus.SUCCESS, message="Success")
